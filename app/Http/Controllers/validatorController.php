...
abstract class validatorController extends Controller {
    ...
    protected function convertir_a_letras($numero) {
        global $importe_parcial;
        ...
        $num_letras .= " "; // aqui iba el asunto de DOLARES.
        return $num_letras;
    }

    protected function millon() {
        global $importe_parcial;
    ...
    protected function cien() {
        ...
        //echo "antes " . $importe_parcial;

        while (substr($importe_parcial, 0, 1) == 0 &&
        substr($importe_parcial, 0, 1) != "" &&
        substr($importe_parcial, 0, 1) != ".") {
            $importe_parcial = substr($importe_parcial, 1, strlen($importe_parcial) - 1);
            //echo "en el while " . $importe_parcial;
        }
        //echo "</br> despues " . $importe_parcial;
    ...
    protected function centena($numero) {
        if ($numero >= 100) {
    ...
    protected function decena($numero) {
    ...
    protected function unidad($numero) {
        $num = "";
        switch ($numero) {
            case 9: {
                    $num = "nueve";
    ...
    protected function centimos() {
        global $importe_parcial;

        $importe_parcial = number_format($importe_parcial, 2, ".", "") * 100;

        if ($importe_parcial > 0)
            $num_letra = " Colones con " . $importe_parcial . '/100';
        else
            $num_letra = " Colones";
...
//
//    protected function decena_centimos($numero) {
//        if ($numero >= 10) {
//            if ($numero >= 90 && $numero <= 99) {
//                if ($numero == 90)
...
//    }
//
//    protected function unidad_centimos($numero) {
//        switch ($numero) {
//            case 9: {
